...
def main():
    try:
        output = subprocess.check_output(
            ['git', 'branch', '-a'], stderr=subprocess.STDOUT, text=True)
        print(f"Git branches:\n{output}\n")
        output_list = output.replace(
            '-> origin/master', '').strip().split('\n')
        ...
        for branch in branch_list:
            try:
                output = subprocess.check_output(
                    ['git', 'checkout', branch], stderr=subprocess.STDOUT, text=True)
        ...
        json.dump(docker_brew_fedora_list, open(
            '../docker_brew_fedora_list.json', 'w'), indent=4)
